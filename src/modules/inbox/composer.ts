...
export function getConversationOpeningTemplateOptions(seeds: ConversationTemplateSeed[]): ConversationOpeningTemplateOption[] {
  ...
  return Array.from(unique.values()).sort((left, right) => (
    left.name.localeCompare(right.name, 'es') || left.languageCode.localeCompare(right.languageCode, 'es')
  ));
}

export function getConversationComposerState(lastInboundAt: Date | null, now = new Date()): ConversationComposerState {
  ...
  if (!lastInboundAt) {
    return {
      ...
      notice: 'Todavía no podés enviar texto libre: no hay actividad entrante registrada.',
      placeholder: 'Cuando haya actividad entrante o exista una plantilla, vas a poder continuar desde acá.',
    };
  }

  if (lastInboundAt.getTime() + whatsappWindowMs <= now.getTime()) {
    return {
      mode: 'template_only',
      canSendFreeText: false,
      notice: 'La ventana de 24 horas está cerrada. Prepará una plantilla para retomar la conversación.',
      placeholder: 'La respuesta libre está bloqueada. Próximo paso: enviar una plantilla aprobada.',
    };
  }
...
type SendConversationTextMessageDeps = {
  ...
  sendText: (input: { to: string; body: string; context?: { message_id: string } }) => Promise<{ messages?: Array<{ id?: string }> } & Record<string, unknown>>;
...
type SendConversationTemplateMessageDeps = {
  ...
  sendTemplate: (input: { to: string; templateName: string; languageCode: string }) => Promise<{ messages?: Array<{ id?: string }> } & Record<string, unknown>>;
  updateConversation: (input: { id: string; lastMessageAt: Date }) => Promise<unknown>;
  createMessage: (input: {
    wamid?: string;
    conversationId: string;
...
type SendConversationDocumentMessageDeps = {
  findConversation: (conversationId: string) => Promise<ConversationRecord | null>;
  uploadMedia: (input: { file: File; filename: string; mimeType: string }) => Promise<{ id: string }>;
  sendMedia: (input: { to: string; type: 'document' | 'image'; mediaId: string; filename: string; caption?: string }) => Promise<{ messages?: Array<{ id?: string }> } & Record<string, unknown>>;
  persistAttachmentMessage: (input: PersistConversationDocumentMessageInput) => Promise<QuotedMessageState | void>;
  writeAuditLog: (input: {
    userId: string;
    action: string;
    entityType: string;
...
type SendConversationDocumentCompatDeps = {
  ...
  sendDocument: (input: { to: string; mediaId: string; filename: string; caption?: string }) => Promise<{ messages?: Array<{ id?: string }> } & Record<string, unknown>>;
...
export type SendConversationTemplateMessageResult =
  ...
  | { ok: false; blockedReason: 'missing_template' | 'not_found' | 'template_unavailable'; notice: string };

export type SendConversationDocumentMessageResult =
  | { ok: true; blockedReason: null; message?: QuotedMessageState }
  | { ok: false; blockedReason: 'missing_file' | 'not_found' | 'template_only' | 'unsupported_type'; notice: string; message?: QuotedMessageState };
...
function defaultDeps(): SendConversationTextMessageDeps {
  return {
    findConversation: (conversationId) => prisma.conversation.findUnique({
      ...
      select: { id: true, contact: { select: { id: true, waId: true, lastInboundAt: true } } },
    }),
    findQuotedMessage: ({ conversationId, messageId }) => prisma.message.findFirst({
      where: { id: messageId, conversationId },
      select: { id: true, wamid: true, direction: true, type: true, body: true, caption: true },
    }),
    findRetryMessage: ({ conversationId, messageId }) => prisma.message.findFirst({
      where: { id: messageId, conversationId },
    ...
    updateConversation: ({ id, lastMessageAt }) => prisma.conversation.update({ where: { id }, data: { lastMessageAt } }),
    createMessage: ({ wamid, conversationId, contactId, body, sentAt, rawJson, status }) => prisma.message.create({
      data: {
        wamid,
        conversationId,
        contactId,
    ...
    updateMessage: ({ id, wamid, body, sentAt, rawJson, status }) => prisma.message.update({
      where: { id },
      data: {
        wamid,
        body,
...
function defaultTemplateDeps(): SendConversationTemplateMessageDeps {
  return {
    ...
    updateConversation: ({ id, lastMessageAt }) => prisma.conversation.update({ where: { id }, data: { lastMessageAt } }),
    createMessage: ({ wamid, conversationId, contactId, body, sentAt, rawJson }) => prisma.message.create({
      data: {
        wamid,
        conversationId,
        contactId,
...
export function getSupportedConversationAttachment(file: { type: string; name: string }): SupportedConversationAttachment | null {
  ...
  if (mimeType === 'image/jpeg' || normalizedFilename.endsWith('.jpg') || normalizedFilename.endsWith('.jpeg')) {
    return {
      kind: 'image',
      messageType: 'IMAGE',
      sendMediaType: 'image',
...
function defaultAttachmentDeps(): SendConversationDocumentMessageDeps {
  return {
    ...
    uploadMedia: ({ file, filename, mimeType }) => createWhatsAppCloudClient().uploadMedia({ file, filename, mimeType }),
    sendMedia: ({ to, type, mediaId, filename, caption }) => createWhatsAppCloudClient().sendMedia({
      to,
      type,
      mediaId,
      filename,
    ...
    persistAttachmentMessage: async ({ conversationId, contactId, wamid, messageType, body, caption, sentAt, rawJson, mediaId, mimeType, filename, size, bytes }) => {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        await tx.conversation.update({ where: { id: conversationId }, data: { lastMessageAt: sentAt } });
      });
    },
    writeAuditLog: (input) => writeAuditLog(input),
    now: () => new Date(),
...
export async function sendConversationTextMessage(
  ...
  const retryMessage = input.retryMessageId && deps.findRetryMessage
    ? await deps.findRetryMessage({ conversationId: input.conversationId, messageId: input.retryMessageId })
    : null;
  const body = requestedBody || retryMessage?.body?.trim() || '';
  if (!body) {
    return { ok: false, blockedReason: 'empty_body', notice: 'Escribí un mensaje antes de enviarlo.' };
  }

  const conversation = await deps.findConversation(input.conversationId);
  if (!conversation) {
    return { ok: false, blockedReason: 'not_found', notice: 'La conversación ya no está disponible.' };
  }

  const composerState = getConversationComposerState(conversation.contact.lastInboundAt, deps.now());
  if (!composerState.canSendFreeText) {
    return { ok: false, blockedReason: 'template_only', notice: composerState.notice };
  }

  const sentAt = deps.now();
  const retryRawJson = retryMessage?.rawJson && typeof retryMessage.rawJson === 'object'
    ? retryMessage.rawJson as { quotedWamid?: string }
    : null;

  const quotedMessage = input.quotedMessageId
    ? await deps.findQuotedMessage({ conversationId: conversation.id, messageId: input.quotedMessageId })
    : null;
  ...
  const persisted = retryMessage && deps.updateMessage
  ...
  if (sendFailed) {
    return {
      ...
      notice: 'El mensaje no pudo enviarse a WhatsApp. Quedó registrado como pendiente de reintento.',
      message,
    };
  }
...
export async function sendConversationTemplateMessage(
  ...
  if (!selectedTemplate) {
    return { ok: false, blockedReason: 'missing_template', notice: 'Elegí una plantilla para reabrir la conversación.' };
  }
  ...
  const template = availableTemplates.find((item) => item.name === selectedTemplate.name && item.languageCode === selectedTemplate.languageCode);
  if (!template) {
    return { ok: false, blockedReason: 'template_unavailable', notice: 'La plantilla elegida ya no está disponible para esta apertura.' };
  }
  ...
  const body = renderTemplateBody(template.body) || `Plantilla enviada: ${template.name} (${template.languageCode})`;
...
export async function sendConversationDocumentMessage(
  ...
  deps: SendConversationDocumentMessageDeps | SendConversationDocumentCompatDeps = defaultAttachmentDeps(),
): Promise<SendConversationDocumentMessageResult> {
  const file = input.file;
  if (file && file.size > 0) {
    const mimeType = file.type.trim().toLowerCase();
    if (mimeType !== 'application/pdf' && !file.name.trim().toLowerCase().endsWith('.pdf')) {
      return { ok: false, blockedReason: 'unsupported_type', notice: 'Por ahora podés adjuntar PDF, JPG o PNG desde el Inbox. WhatsApp Cloud API no acepta WEBP como imagen saliente.' };
    }
  }
  ...
  return sendConversationAttachmentMessage(input, {
    ...
    sendMedia: 'sendMedia' in deps ? deps.sendMedia : ({ to, mediaId, filename, caption }) => deps.sendDocument({ to, mediaId, filename, caption }),
    persistAttachmentMessage: 'persistAttachmentMessage' in deps ? deps.persistAttachmentMessage : deps.persistDocumentMessage,
...
export async function sendConversationAttachmentMessage(
  ...
  const file = input.file;
  if (!file || file.size <= 0) {
    return { ok: false, blockedReason: 'missing_file', notice: 'Elegí una imagen o un PDF antes de enviarlo.' };
  }
  ...
  await deps.writeAuditLog({
    userId: input.session.userId,
    action: attachment.kind === 'image' ? AUDIT_ACTIONS.INBOX_IMAGE_SENT : AUDIT_ACTIONS.INBOX_DOCUMENT_SENT,
    entityType: 'conversation',
    entityId: conversation.id,
...
export async function sendConversationAttachmentMessages(
  ...
  sendAttachment: (input: SendConversationDocumentMessageInput) => Promise<SendConversationAttachmentMessageResult> = (singleInput) => sendConversationAttachmentMessage(singleInput),
  ...
  for (const [index, file] of files.entries()) {
    ...
    if (!result.ok) {
      return { ok: false, blockedReason: result.blockedReason, notice: result.notice, message: result.message };
    }

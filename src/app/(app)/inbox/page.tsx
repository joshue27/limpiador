...
import {
  ...
  canViewConversation,
  conversationListWhereForSession,
} from '@/modules/inbox/access';
import {
  buildConversationSearchResult,
  splitTextSearchMatches,
  type ConversationSearchMatch,
} from '@/modules/inbox/chat-search';
...
import { InlineEditableName } from '@/modules/inbox/InlineEditableName';
...
import {
  ...
  inboxLink,
  parseInboxFilters,
} from '@/modules/inbox/list-filters';
...
function assignmentLabel(conversation: {
...
export default async function InboxPage({
  ...
  const visibleDepartmentsPromise =
    session.role === 'ADMIN'
  ...
  const [visibleUsers, visibleDepartments, quickContacts] = await Promise.all([
    visibleUsersPromise,
    visibleDepartmentsPromise,
    quickContactsPromise,
  ]);
  ...
  if (filters.status)
    filterWhere.push({ status: filters.status as Prisma.EnumConversationStatusFilter['equals'] });
  ...
  const where: Prisma.ConversationWhereInput = filterWhere.length
  ...
  const selectedId =
    requestedId && (await canViewConversation(session, requestedId))
      ? requestedId
      : conversations[0]?.id;
  const selected = selectedId
    ? await prisma.conversation.update({
        ...
        include: {
          ...
          contact: true,
          messages: {
            where: { hiddenGlobally: false, hiddenByUsers: { none: { userId: session.userId } } },
  ...
  const canTransferSelected = selected
    ? await canTransferConversation(session, selected.id)
    : false;
  const departments =
    selected && canTransferSelected
  ...
  const queueCount = conversations.filter(
    (conversation) => conversation.status === 'DEPARTMENT_QUEUE' && !conversation.assignedToId,
  ).length;
  const claimedCount = conversations.filter(
    (conversation) => conversation.status === 'CLAIMED' || conversation.assignedToId,
  ).length;
  const summaryBar = getInboxSummaryBarModel({
  ...
  const hasChatSearch =
    typeof params?.chatSearch === 'string' && params.chatSearch.trim().length > 0;
  const fullConversationSearchMessages =
    hasChatSearch && selected
  ...
  const activeFilterLabels = [
    ...
    filters.assignedUser
      ? `Usuario: ${visibleUsers.find((user) => user.id === filters.assignedUser)?.email ?? 'Aplicado'}`
      : null,
    filters.department
      ? `Depto: ${visibleDepartments.find((department) => department.id === filters.department)?.name ?? 'Aplicado'}`
      : null,
    filters.tag
      ? `Etiqueta: ${activeTags.find((item) => item.code === filters.tag)?.name ?? filters.tag}`
      : null,
  ].filter(Boolean);
      ...
      <section className="inbox-layout">
        <aside className="card conversation-list" aria-label="Bandeja de conversaciones">
          ...
          {chatNotice ? (
            <p
              className={
                chatNoticeType === 'error'
                  ? 'notice notice-error inbox-inline-notice'
                  : 'notice inbox-inline-notice'
              }
            >
              {chatNotice}
            </p>
            ...
            <summary>
              ...
              <span className="filter-summary-pills">
                {activeFilterCount > 0 ? (
                  ...
                  <span className="filter-pill muted">Sin filtros</span>
                )}
              </span>
            </summary>
            <form
            ...
            >
              ...
              <label>
                ...
                <select name="status" defaultValue={filters.status}>
                  ...
                  {conversationStatusOptions.map(([value, label]) => (
              ...
              <div className="inbox-filter-actions">
                ...
                <a href="/inbox" className="button-link-secondary">
                  Limpiar
                </a>
              </div>
            </form>
          ...
          {conversations.length === 0 ? (
            <p className="empty-state">
              No hay conversaciones para el usuario actual. Cuando ingrese un mensaje, aparecerá en
              esta bandeja.
            </p>
          ) : null}
          {conversations.map((conversation) => {
            ...
            return (
              <Link
                key={conversation.id}
                href={inboxLink(conversation.id, filters)}
              ...
              >
                ...
                <span className="conversation-preview">
                  {previewMessage(conversation.messages[0])}
                </span>
                <span className="conversation-meta">
                  <span
                    className={`status-pill status-${conversation.status.toLowerCase().replaceAll('_', '-')}`}
                  >
                    {labelFor(conversationStatusLabels, conversation.status)}
                  </span>
                  {compactTags(conversation.contact.tags).map((contactTag) => (
                    <span key={contactTag} className="tag-chip">
                      {tagLabel(activeTagNames, contactTag)}
                    </span>
                  ))}
                  <span className={`status-pill ${windowState.className}`}>
                    {windowState.label}
                  </span>
          ...
          {selected ? (
            (() => {
              ...
              const composerLayout = getInboxComposerLayoutModel({
                composerState,
                hasTemplates: openingTemplates.length > 0,
              });
              const actionMenu = getConversationActionMenuModel({
                canClaim: canClaimSelected,
                canTransfer: canTransferSelected,
              });
              const selectableTags = activeTags.filter(
                (tagItem) => !selected.contact.tags.includes(tagItem.code),
              );
              return (
                <>
                  <header className="conversation-header">
                    ...
                    <div className="conversation-facts">
                      <span
                        className={`status-pill status-${selected.status.toLowerCase().replaceAll('_', '-')}`}
                      >
                        {labelFor(conversationStatusLabels, selected.status)}
                      </span>
                      <span className={`status-pill ${selectedWindowState.className}`}>
                        {selectedWindowState.label}
                      </span>
                      <small>{selectedWindowState.detail}</small>
                      <span className="conversation-fact-chip">{assignmentLabel(selected)}</span>
                      <span className="conversation-fact-chip">
                  ...
                  >
                    <div className="actions-grid">
                      {canClaimSelected ? (
                        <form
                          action={`/api/inbox/${selected.id}/claim`}
                          method="post"
                          className="claim-form"
                        >
                          <button type="submit" className="compact-action-button">
                            {actionMenu.primaryActionLabel}
                          </button>
                        </form>
                      ) : null}
                      {session.role === 'ADMIN' && (
                        <>
                          <form
                            action={`/api/inbox/${selected.id}/delete`}
                            method="post"
                            className="delete-form"
                            id={`delete-form-${selected.id}`}
                          >
                      ...
                      <small className="conversation-inline-count" id="message-count">
                        {visibleMessages.length} mensajes
                      </small>
                    </div>
                    <ChatSearchForm
                      conversationId={selected.id}
                    ...
                    {actionMenu.showOverflowMenu ? (
                      <details className="conversation-overflow-menu">
                        ...
                        <div className="conversation-overflow-popover">
                          {canTransferSelected ? (
                            <form
                              action={`/api/inbox/${selected.id}/transfer`}
                              method="post"
                              className="transfer-form overflow-transfer-form"
                            >
                              <label>
                  ...
                  <ConversationChatClient
                    ...
                    isBlurred={
                      session.role !== 'ADMIN' &&
                      selected.assignedToId !== session.userId &&
            ...
            <div className="empty-state inbox-empty">
              ...
              <p>
                Desde la bandeja izquierda puede ver el estado, la asignación y los mensajes sin
                leer. Al elegir una conversación, aquí aparecerán el historial y las acciones
                disponibles.
              </p>
            </div>
          )}

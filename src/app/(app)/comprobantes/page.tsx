...
export default async function ComprobantesPage({ searchParams }: { searchParams?: Promise<{ from?: string; to?: string; status?: string; comprobante?: string; page?: string }> }) {
  const session = await requirePermission('comprobantes');
  const params = await searchParams;
  const currentPage = Math.max(1, Number(params?.page) || 1);
  const status = ['PENDING', 'DOWNLOADING', 'READY', 'FAILED'].includes(params?.status ?? '') ? params?.status : '';
  const comprobante = params?.comprobante === 'yes' ? true : params?.comprobante === 'no' ? false : undefined;
    ...
    <div className="stack" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <section className="card" style={{ flexShrink: 0 }}>
        <form className="grid-form" action="/comprobantes">
          ...
          <select name="comprobante" defaultValue={params?.comprobante ?? ''} aria-label="Filtro archivado">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12, flexShrink: 0 }}>
          ...
          {totalPages > 1 && (
            <div className="csv-pagination">
              {currentPage > 1 && <a href={buildPageUrl(urlParams, currentPage - 1)} className="button-secondary" style={{ fontSize: '0.8rem' }}>← Anterior</a>}
              <small>Pág {currentPage} de {totalPages}</small>
              {currentPage < totalPages && <a href={buildPageUrl(urlParams, currentPage + 1)} className="button-secondary" style={{ fontSize: '0.8rem' }}>Siguiente →</a>}
            </div>
          )}
        </div>
        <div className="table-card" style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
          {media.length ? <table>
            <thead><tr><th>Contacto</th><th>Archivo</th><th>Estado</th><th>Archivado</th><th>Descarga</th><th></th></tr></thead>
            <tbody>
              {media.map((asset) => (
                <tr key={asset.id} id={asset.id}>
                  ...
                  <td>
                    <form action={toggleArchivado} className="mini-form">
                      ...
                      <input type="hidden" name="isComprobante" value={String(!asset.isComprobante)} />
          ...
          </table> : <p className="empty-state">No hay adjuntos para estos filtros. Cuando lleguen imágenes, audios o documentos por WhatsApp, aparecerán aquí.</p>}
        </div>
      </section>
    </div>
  );

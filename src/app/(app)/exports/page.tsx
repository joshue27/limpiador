  ...
  return (
    <div className="stack" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <section style={{ flexShrink: 0 }}>
        ...
        <h2>Exportaciones</h2>
        <p>
          Solicitá la exportación de archivados por rango de fechas y descargá el ZIP cuando esté
          listo.
        </p>
      </section>
        ...
        >
          ...
          {totalPages > 1 && (
            <div className="csv-pagination">
              {currentPage > 1 && (
                <a
                  href={buildPageUrl(currentPage - 1, params.from, params.to)}
        ...
        <div className="table-card" style={{ flex: 1, overflow: 'auto', minHeight: 0 }}>
          {runs.length ? (
            <table>
              ...
              <tbody>
                {runs.map((run) => {
                  const counts = run.countsJson as {
                    total?: number;
                    size?: number;
                    error?: string;
                  } | null;
                  return (
                    <tr key={run.id}>
                      ...
                      <td>
                        ...
                        {counts?.error && <br />}
                        {counts?.error && (
                          <small style={{ color: '#dc2626', fontSize: '0.65rem' }}>
                            {counts.error}
                          </small>
                        )}
                        {run.status === 'RUNNING' && (
            ...
            <p className="empty-state">
              No hay exportaciones todavía. Solicitá una con el formulario de arriba.
            </p>
          )}
        </div>
      </section>

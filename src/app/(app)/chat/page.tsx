...
type ChatUser = { id: string; name: string; online: boolean; departments: string[]; unreadCount: number };
type ChatMessage = {
  id: string; body: string; createdAt: string; userId: string; recipientId: string | null;
...
export default function ChatPage() {
  ...
  async function loadMessages(force = false) {
    ...
    try {
      ...
      if (!res.ok) return;
      const data = await res.json() as { messages: ChatMessage[]; users: ChatUser[]; currentUserId: string; generalUnread: number };
      ...
      if (generalBtn && selectedUser === null) {
        ...
        if (!badge) {
          ...
          badge.style.cssText = 'background:var(--accent,#075e54);color:#fff;font-size:0.6rem;padding:1px 5px;border-radius:8px;margin-left:4px';
      ...
      if (lastCountRef.current > 0 && newMessages.length > lastCountRef.current) {
        ...
        if (latest && latest.userId !== data.currentUserId) {
          ...
          if (!muted) {
            try {
              const audio = new Audio('/notification-message.mp3?' + Date.now());
              audio.volume = 0.5;
              audio.play().catch(() => {});
            } catch { /* */ }
          }
  ...
  return (
    <div style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="card" style={{ flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden', padding: 0 }}>
        {/* User list sidebar */}
        <div style={{ width: 180, borderRight: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px 8px' }}>
            ...
            <div style={{ marginTop: 8 }}>
              <small style={{ color: '#9ca3af', fontSize: '0.65rem', textTransform: 'uppercase', padding: '0 8px' }}>
              ...
              .map((user) => (
                ...
                >
                  <span style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.78rem' }}>
                    <span style={{ fontSize: '0.5rem', color: user.online ? '#10b981' : '#d1d5db' }}>●</span>
                    {user.name}
                    {user.unreadCount > 0 && (
                      <span style={{
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
          <div style={{ padding: '6px 12px', borderBottom: '1px solid #e5e7eb', fontSize: '0.8rem', color: '#6b7280', flexShrink: 0 }}>
              ...
              return (
                <div key={msg.id} style={{ display: 'flex', flexDirection: 'column', alignItems: isMe ? 'flex-end' : 'flex-start' }}>
                  ...
                  <div style={{
                    ...
                    padding: '6px 12px', borderRadius: 12, maxWidth: '70%',
          ...
          <form onSubmit={send} style={{ display: 'flex', gap: 8, padding: '8px 12px', borderTop: '1px solid #e5e7eb', flexShrink: 0 }}>
            <input value={body} onChange={e => setBody(e.target.value)} placeholder="Escriba un mensaje…" style={{ flex: 1 }}
              onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); void send(e as unknown as React.FormEvent); } }}
            />
            <button type="submit" disabled={sending || !body.trim()} className="compact-action-button">
              {sending ? '…' : 'Enviar'}
            </button>
          </form>
        </div>
      ...
      {toast && (
        <div className="notification-toast" onClick={() => setToast(null)} style={{ bottom: 70 }}>
          ...
          <button type="button" onClick={(e) => { e.stopPropagation(); setToast(null); }}>×</button>
        </div>
      )}
    </div>
  );

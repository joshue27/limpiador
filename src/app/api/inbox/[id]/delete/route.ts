...
export async function POST(
  ...
  if (confirmation !== 'ELIMINAR') {
    return NextResponse.json({ 
      error: 'Se requiere confirmación explícita. Escriba "ELIMINAR" para confirmar la eliminación.' 
    }, { status: 400 });
  }

  try {
    // Fetch media assets to clean up files from disk
    const mediaAssets = prisma.mediaAsset ? await prisma.mediaAsset.findMany({
    ...
    return NextResponse.json({ error: 'Error interno al eliminar la conversación.' }, { status: 500 });
  }
}
